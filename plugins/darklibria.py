...
class darklibria:
    ...
    def handle_page(self, what, page):
        ...
        with ThreadPoolExecutor() as executor:
            for serial in serials:
                link_tag = serial.find('a')
                if not link_tag or 'href' not in link_tag.attrs:
                    continue
                executor.submit(self.handle_serial, link_tag['href'])
        return parser
    ...
    def handle_serial(self, url):
        ...
        if not data:
            return
        parser = Parser(data)
        name_tag = parser.find(attrs={'id': 'russian_name'}) or parser.find(attrs={'id': 'english_name'})
    ...
    def handle_torrent_row(self, torrent_row, name, url):
        children = list(torrent_row.children)
        if len(children) < 7:
            return
        type, quality, size_data, date_time, download, seeds, leech, *_ = children
        self.pretty_printer({
    ...
    def get_link(self, download):
        ...
        if magnet and 'href' in magnet.attrs:
            return magnet['href']
        torrent = download.find(attrs={'title': 'Скачать торрент'})
        if torrent and 'href' in torrent.attrs:
            return torrent['href']
        return ''
    ...
    def get_date(self, date_time):
        match = self.dt_regex.search(date_time.text)
        if not match:
            return date_time.text.strip()
        utc_dt_string = match.group()
        utc = datetime.strptime(utc_dt_string, '%Y-%m-%d %H:%M:%S')
        return str(utc2local(utc))

    def get_size(self, size_data):
        parts = size_data.text.split()
        if len(parts) < 2:
            return size_data.text.strip()
        size, unit = parts[0], parts[1]
        return size + ' ' + self.units_dict.get(unit, unit)
